function Main(input: string[]) {
  ...
  const countTwoDivided: number[] = [];
  const countThreeDivided: number[] = [];
  let remain = 1;
  let minTwoDivided = 1000000000,
    minThreeDivided = 1000000000;
  ...
  let ans = countTwoDivided.reduce(
    (sum, count) => (sum += count - minTwoDivided),
    0
  );
  ans += countThreeDivided.reduce(
    (sum, count) => (sum += count - minThreeDivided),
    0
  );
