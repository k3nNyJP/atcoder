function calcDistance(s1: string, s2: string): number {
  return s1
    .split("")
    .reduce(
      (distance, char, index) => (s2[index] === char ? distance : distance + 1),
      0
    );
}

function search(current: string, remains: string[]): boolean {
