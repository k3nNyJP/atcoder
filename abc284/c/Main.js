function Main(input) {
    ...
    for (let i = 1; i <= M; i++) {
        ...
        const rightGroupIndex = groups.findIndex((group) => group.includes(edge[1]));
        if (leftGroupIndex === rightGroupIndex) {
            continue;
        }
        const leftGroup = groups.splice(leftGroupIndex, 1)[0];
        const rightGroup = groups.splice(leftGroupIndex > rightGroupIndex ? rightGroupIndex : rightGroupIndex - 1, 1)[0];
