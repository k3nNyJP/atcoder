function Main(input: string[]) {
  ...
  const resultList: { index: number; math: number; english: number }[] = [];
  const passedList: { index: number; math: number; english: number }[] = [];
  ...
  if (Z > 0) {
    resultList.sort(
      (a, b) => b.math + b.english - (a.math + a.english) || a.index - b.index
    );
