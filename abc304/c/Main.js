function Main(input) {
    ...
    for (let i = 2; i <= N; i++) {
        ...
        const infectedGroupIndexList = [];
        for (let pandemicGroupIndex = 0; pandemicGroupIndex < pandemicGraph.length; pandemicGroupIndex++) {
            ...
            for (let j = 0; j < pandemicGroup.length && !infected; j++) {
                ...
                const distance = Math.sqrt((parsonI[0] - parsonJ[0]) ** 2 + (parsonI[1] - parsonJ[1]) ** 2);
                if (distance <= D) {
                    infected = true;
                }
            }
        ...
        while (infectedGroupIndexList.length > 1) {
            ...
            const concatTargetIndex = infectedGroupIndexList[infectedGroupIndexList.length - 1];
